...
from single_player import grid1, grid2
...
class SinglePlayerTest(unittest.TestCase):
    def test_put_one_ship(self):
        ...
        self.assertEqual(grid1[0], [5, 5, 5, 0, 0, 0, 0, 0, 0, 0])
    ...
    def test_ignore_ships_outside_the_sea(self):
        player1 = single_player.player1
        self.assertFalse(single_player.put_one_ship(player1, [11, 11], 5, 0))
        self.assertEqual(grid1[0], [5, 5, 5, 0, 0, 0, 0, 0, 0, 0])

    def test_player_make_move(self):
        single_player.put_one_ship(single_player.player1, [0, 0], 3, 0)
        single_player.player.game.Ship(3, single_player.sea2, [8, 0], 0)
        self.assertTrue(single_player.player_make_move([300, 189]))
        single_player.player_make_move([323, 189])
        single_player.player_make_move([346, 189])
        self.assertEqual(grid2[8][0], 1)
        self.assertFalse(single_player.player_make_move([367, 215]))
    ...
    def test_computer_make_move(self):
        single_player.put_one_ship(single_player.player1, [8, 8], 3, 0)
    ...
    def test_computer_put_his_ships(self):
        ...
        number_of_parts = 0
        for row in single_player.sea2.board:
